...
4. 生成：单文件 index.html（CSS/JS 内联，零外部依赖，字体系统栈回退）；真实业务文案与样本数据，不用 Lorem；token 外不许散写 hex；八态逐页覆盖；`docs/UI-QUALITY-FLOOR.md` 的 MUST 不宣告地做到；`docs/DESIGN-VOCABULARY.md` 通病清单逐条对照，命中的说出改了什么。
...
7. 交付：落盘 `demo/`；编码参照优先级 demo → token → Brief → Spec；报告写清覆盖/验收/已知缺口。
