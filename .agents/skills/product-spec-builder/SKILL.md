...
3. 问清业务（只问会改变条目的事，分四面：目标链路量级角色例外 / 每条规则的判据加例子 / 用户原话翻成接收方语言 / 旧规则能否沿用）。先自己查（代码/旧 REQ/竞品/联网可核的），查不到的才问人；问过去发生的具体事，不问未来假设；听到"一般/通常"或形容词必须追一个具体例子；每确认一条当场复述锁定，问完就等不连珠炮；矛盾双方并排摆出请用户排序，对比后给推荐+理由（可单选可组合折中，可逆细节按推荐直接推进）。通用追问方法走原生 grill。
