...
**已知缺口（2026-09-19，Fable5.1 评审）**：18 个主入口是产物/步骤清单，对话判断（何时停问、何时顶回、好问答长什么样）缺失——主入口 0 句，references 合计 1 句（`arch-designer` 实测）。原因：骨架搭在读源码之前（见上更正记录），压缩时无使用依据。

**改令（2026-09-19，用户）**：Fable #2 收窄已回退，18 skill 全数装机（b9b754c 视为废弃过渡态）。"犯错才加""不回填"约束作废——donor 原文可直接采用。保留：#1 行为对照方法与基线（见 `progress.md` §对照）。
