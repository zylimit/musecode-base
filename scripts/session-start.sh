...
elif [ -z "$has_plan" ] && [ -z "$has_code" ]; then echo "=> Spec 已完成：出开发计划（dev-planner 已下架到 .agents/parts/，按需取用）";
elif [ -n "$has_plan" ] && [ -z "$has_code" ]; then echo "=> Plan 已完成：走 dev-builder 开工";
elif [ -z "$has_plan" ]; then echo "=> 缺计划：建议补一份（dev-planner 已下架到 .agents/parts/，按需取用）";
