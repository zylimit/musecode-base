...
# 对照注册清单算出：哪些门禁真拦过（有战绩）、哪些注册了却从没出现
...
# 这样一次 find 既覆盖主仓 .agents/harness-state 又覆盖所有 worktree 下的同名账本。
common_dir=$(git rev-parse --path-format=absolute --git-common-dir 2>/dev/null || true) # musecode-fitness:ignore no-silent-failure reason="non-git scan root falls back below"
...
# 2) 注册闸清单：只认源码里写账本的脚本（调 gate_log 的门禁）才算闸。
#    只读诊断类脚本（本文件、test-age 等不写账本、永不拦截）不是闸，不纳入——
#    否则它们必然「零记录」，被误报成死闸。现算，别写死名单：写死的名单一定会跟代码走散。
registered=()
while IFS= read -r f; do registered+=("$(basename "$f" .sh)"); done \
  < <(grep -lE "gate_log\(" "$scan_root/scripts/"*.sh 2>/dev/null | sort -u)
...
echo "════════════════════════════════════════════════════════════════"
echo " 门禁拦截战绩审计（gate-audit）"
echo " 扫描根：$scan_root"
echo "════════════════════════════════════════════════════════════════"
echo ""

# (a) 有战绩的钩子
echo "── (a) 有战绩的门禁（按拦截次数降序）──"
if [ -n "$stats" ]; then
...
echo "── (b) 零记录门禁（注册了但从没拦过：前置条件不在本仓，或威慑生效）──"
zero_count=0
